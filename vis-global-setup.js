...
function fixOffset(e, chart) {
  // console.log(e)
  const pBack = document
    .getElementById(e.renderTo.id)
    .getElementsByClassName('highcharts-plot-background')[0];
  const pBackW = pBack.width.baseVal.value;

  const diff = singlePlotWidth - pBackW;
  ...
  const { plotHeight } = e;
  const hDiff = singlePlotHeight - plotHeight;
